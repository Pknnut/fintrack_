...
function renderSafeToSpend() {
  ...
  // Active instalment monthly burden — excludes any instalment already covered
  // this month. lastPaidYM alone isn't reliable: it's only set by the dedicated
  // "Paid This Month" button, so a payment logged through the regular Add form
  // instead (matching category/description but bypassing that button) would leave
  // lastPaidYM stale, and the same real payment got subtracted here as "still
  // due" AND already counted in spentSoFar above — double-counted. Recurring
  // already avoids this exact trap by checking actual transaction content instead
  // of an internal flag (see getPendingRecurring/isLoggedThisMonth); mirror that
  // here: an instalment counts as covered if lastPaidYM matches, OR a tagged
  // fromInst transaction exists this month, OR any expense this month's
  // description matches this instalment's name.
  const instDue = INSTALLMENTS.filter(p => {
    if (p.paid >= p.total_mo) return false;
    if (p.lastPaidYM === curYM) return false;
    const nameKey = (p.name || "").trim().toLowerCase();
    const coveredByTx = mt.some(t => t.type === "Expense" && (
      (t.fromInst && t.instId === p.id) ||
      (nameKey && String(t.desc || t.description || "").toLowerCase().includes(nameKey))
    ));
    return !coveredByTx;
  }).reduce((s,p)=>s+(p.monthly||0), 0);
